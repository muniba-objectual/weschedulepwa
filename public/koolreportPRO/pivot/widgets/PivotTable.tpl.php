            ...
            <?php if ($i === 0) { ?>
                <td class='pivot-data-field-zone'
                    ...
                    <div class='pivot-data-field-content'>
                        <?php echo $mappedDataFieldZoneValue; ?>
                    </div>
                </td>
            <?php }
        ...
        if ($this->showDataHeaders) { ?>
            ...
            foreach ($colIndexes as $c => $j) {
                ...
                foreach($dataFields as $di => $df) { 
                    $mappedDH = $mappedDataHeaders[$j][$df]; 
        ...
        foreach($rowIndexes as $r => $i) {
            ...
            <tr class='pivot-row'>
                ...
                foreach ($colIndexes as $c => $j) {
                    ...
                    $dataRow = $this->pivotUtil->getDataForCell($i, $j);
                    list($mappedDataRow, $dataRowClass, $excelStyle, $spreadsheetStyle) =
                        $this->pivotUtil->getDataAttributesForCell($i, $j);
                    // print_r($mappedDataRow); echo "<br>";
                    // print_r($dataRow); echo "<br>";
                    foreach($dataFields as $di => $df) {
                        $value = Util::get($dataRow, $df, $emptyValue); ?>
    ...
    KoolReport.widget.init(
        ...
        function() {
            ...
            var <?=$uniqueId?>_data = {
                ...
                rowTotalAtBeginning: <?php echo $this->rowTotalAtBeginning ? 1 : 0; ?>,
                columnTotalAtBeginning: <?php echo $this->columnTotalAtBeginning ? 1 : 0; ?>,
