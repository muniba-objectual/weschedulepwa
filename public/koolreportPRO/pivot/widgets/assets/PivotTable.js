...
KoolReport.PivotTable = KoolReport.PivotTable || (function () {
    ...
    function expandCollapseRow(e) {
        ...
        while (el) {
            var shouldChangeLayer = true;
            if (this.rowTotalAtBeginning && el.dataset.value !== "{{all}}") shouldChangeLayer = false;
            if (shouldChangeLayer) this.changeLayer(el, expand);
            if (this.rowTotalAtBeginning && el.classList.contains('pivot-data-cell')) {
        ...
        while (i < 1 * td.rowSpan) {
            ...
            for (var j = 0; j < el.children.length; j += 1) {
                ...
                if (!this.rowTotalAtBeginning && child.classList.contains('pivot-data-cell') &&
                    i === td.rowSpan - 1) {
                    child.classList.toggle('pivot-data-cell-row-total');
                    shouldChangeLayer = false;
                }
                if (shouldChangeLayer) this.changeLayer(child, expand);
            }
            i += 1;
        }
