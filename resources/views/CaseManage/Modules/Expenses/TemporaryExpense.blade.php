...
@section('content_header')
    <h1 class="m-0 text-dark">View Expense for

        @php
            if ($expenseId) {
                $tmpExpense = \App\Models\Expenses::where('id','=',$expenseId)->first();
                if ($tmpExpense) {
                    $tmpUser = \App\Models\User::where('id','=',$tmpExpense->fk_UserID)->first();
                    if ($tmpUser) {
                        echo ($tmpUser->name);
                        }
                    }
        ...
        <div class="float-right">
            <a href="javascript:history.back();"><i class="fas fa-arrow-left text-primary"></i></a>
        </div>
    </h1>
    <script>
        $userID = {{$user->id}};
                ...
                <div class="card">
                    ...
                    <div>
                        <a href="#" class="btn btn-sm" style="float:right;margin-top: 1em;/*position:fixed;right: 40px;*/"
                           ...
                           onclick="Livewire.emit('slide-over.open', 'notifications.expense-action-center')"
                        >
                            <i class="fa-solid fa-bell fa-xl" style="color: orangered;"></i>
                        </a>
                    </div>
                    <div class="card-body">
                        @livewire('expenses-report.standalone-expense-component', ['expenseId' => $expenseId])
                    </div>
                </div>
            </div>
        </div>
