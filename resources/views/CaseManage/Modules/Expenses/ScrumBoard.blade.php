...
@section('content_header')
    <h1 class="m-0 text-dark">Expenses Summary Report</h1>
...
@section('content')
    ...
    <div class="container-fluid" style="width:100%; height:85%; overflow-y:auto;">
        <div class="row">
            <div class="col-12">
                @if(count($tabCollection)>1)
                    <div class="card-header p-2">
                        <ul class="nav nav-pills">
                            @foreach($tabCollection as $tabHead)
                                <li class="nav-item">
                                    <a class="nav-link {{session('expense.report.active-tab')==$tabHead?'active':''}}" href="{{ route('expense.scrum-board', ['tab_key' => $tabHead]) }}">{{$tabHead}}</a>
                                </li>
                            @endforeach
                        </ul>
                    </div>
                ...
                <div class="card">
                    <div class="card-body">
                        @livewire('expenses-report.dynamic-scrum-board-component', [])
                    </div>
                </div>
            </div>
        </div>
