<div>
        <style>
            body {
                font-family: Arial, sans-serif;
                font-size: 14px;
            }

            .header {
                text-align: center;
                font-size: 18px;
                font-weight: bold;
                margin-bottom: 20px;
            }

            .section-heading {
                font-size: 16px;
                font-weight: bold;
                margin-top: 20px;
                margin-bottom: 10px;
            }

            .label {
                font-weight: bold;
            }

            .input-field {
                border: none;
                border-bottom: 1px dotted #000;
                outline: none;
                padding: 2px;
            }

            .signature {
                margin-top: 50px;
            }

            .logo{
                text-align: center;
            }

            textarea, input[type="text"] {
                background: #f4eef1;
            }
        </style>

    @php
        $config = [
            "singleDatePicker" => true,
            "showDropdowns" => true,
            "startDate" => "js:moment()",
            "minYear" => 2000,
            "maxYear" => "js:parseInt(moment().format('YYYY'),10)",
            "timePicker" => true,
            "timePicker24Hour" => false,
            "timePickerSeconds" => false,
            "cancelButtonClasses" => "btn-danger",
            "locale" => ["format" => "YYYY-MM-DD HH:mm"],
        ];
    @endphp

    <form wire:submit.prevent="submit">

        <div class="logo">
            <img width="166" height="116" src="data:image/jpg;base64,"/>
        </div>

        <div class="header">AUTHORIZATION FOR SUPERVISED ACTIVITIES</div>

        <p>TO: Carpe Diem Residential Treatment Homes for Children Inc.</p>

        <p>RE: Child: <span id="childName"><input type="text" class="input-field" wire:model="formData.child_name"></span></p>

        <p>Date of Birth: <span id="childDOB"><input type="text" class="input-field" wire:model="formData.child_dob"></span></p>

        <p>I/We <input type="text" class="input-field" wire:model="formData.guardian_name">, the Guardian(s)/Parent(s) of the above child, give permission to Carpe Diem Residential Treatment Homes for Children Inc., to allow our/my child to participate in all supervised activities, including trips which may involve public and/or private transportation.</p>

        <p><input type="text" class="input-field" wire:model="formData.signature_1">/<x-adminlte-date-range name="signature_1_date" wire:model="formData.signature_1_date" igroup-size="sm" class="input-field" :config="$config"/>/<input type="text" class="input-field" wire:model="formData.signature_1_name"><br>
            Signature&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Date&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Name (please print)</p>

        <p><input type="text" class="input-field" wire:model="formData.witness_1">/<x-adminlte-date-range name="witness_1_date"  wire:model="formData.witness_1_date" igroup-size="sm" class="input-field" :config="$config"/>/<input type="text" class="input-field" wire:model="formData.witness_1_name"><br>
            Witness&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Date&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Name (please print)</p>

        <p><input type="text" class="input-field" wire:model="formData.signature_2">/<x-adminlte-date-range name="signature_2_date" wire:model="formData.signature_2_date" igroup-size="sm" class="input-field" :config="$config"/>/<input type="text" class="input-field" wire:model="formData.signature_2_name"><br>
            Signature&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Date&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Name (please print)</p>

        <p><input type="text" class="input-field" wire:model="formData.witness_2">/<x-adminlte-date-range name="witness_2_date" wire:model="formData.witness_2_date" igroup-size="sm" class="input-field" :config="$config"/>/<input type="text" class="input-field" wire:model="formData.witness_2_name"><br>
            Witness&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Date&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Name (please print)</p>


        <p class="footer">
            <i>9355 Dixie Road, Brampton, ON L6S 1J7 Tel: 905.799.2947 Fax: 905.790.8262 www.carpediem.ca</i>
        </p>

        <button type="submit">Submit</button>
        <button type="button">Email</button>
    </form>


</div>
