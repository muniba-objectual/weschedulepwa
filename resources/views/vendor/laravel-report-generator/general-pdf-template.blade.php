...
<html lang="en">
	...
	<body>
	    							...
	    							foreach ($columns as $colName => $colData) {
	    								if (array_key_exists($colName, $showTotalColumns)) {
	    									if ($showTotalColumns[$colName] == 'point') {
	    										echo '<td class="center"><b>' . number_format($total[$colName], 2, '.', ',') . '</b></td>';
	    									} else {
	    										echo '<td class="center"><b>' . strtoupper($showTotalColumns[$colName]) . ' ' . number_format($total[$colName], 2, '.', ',') . '</b></td>';
	    									}
	    									$dataFound = true;
	    								} else {
	    									if ($dataFound) {
						...
						<tr class="bg-black f-white">
							...
							@foreach ($columns as $colName => $colData)
								@if (array_key_exists($colName, $showTotalColumns))
									<?php $dataFound = true; ?>
									@if ($showTotalColumns[$colName] == 'point')
										<td class="center"><b>{{ number_format($total[$colName], 2, '.', ',') }}</b></td>
									@else
										<td class="center"><b>{{ strtoupper($showTotalColumns[$colName]) }} {{ number_format($total[$colName], 2, '.', ',') }}</b></td>
									@endif
								@else
									@if ($dataFound)
										<td></td>
	    ...
	    <script type="text/php">
	        ...
	        if ( isset($pdf) ) {
	            $pdf->page_text(30, ($pdf->get_height() - 26.89), "Date Generated: " . date('d M Y H:i:s'), null, 10);
